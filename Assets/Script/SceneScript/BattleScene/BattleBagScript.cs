...
public class BattleBagScript : MonoBehaviour
{
    ...
    MyGridLayout<RoleItem> mMyGridLayout;
    List<RoleItem> datas;
    ...
    private void Start()
    {
        ...
        mMyGridLayout = new MyGridLayout<RoleItem>();
        mMyGridLayout.StartInit(this.gameObject, this.gridItemUIPrefab, myAdapter);
    }

    private void Update()
    {
        this.mMyGridLayout.Update();
    }

    class MyAdapter : Adapter<RoleItem>
    {
        BattleBagScript battleBagScript;
        public MyAdapter(List<RoleItem> datas, BattleBagScript battleBagScript) : base(datas)
        {
            this.battleBagScript = battleBagScript;
        }

        public override void BindView(GameObject gridItem, int index)
        {
            RoleItem roleItem = this.datas[index];
            gridItem.GetComponentInChildren<Text>().text = roleItem.itemId + "_" + roleItem.itemName + "_" + roleItem.itemCount; //todo 测试用id
            gridItem.GetComponentInChildren<Image>().sprite = Resources.Load<Sprite>("Images/ItemImage/" + roleItem.imageName);
        }
        ...
        public override void OnItemClick(GameObject gridItem, int index)
        {
            Debug.LogWarning("OnItemClick " + index);
        }

        public override void OnSelectGridItem(GameObject gridItem, int index)
        {
            Debug.LogWarning("OnSelectGridItem " + index);
            if(index >= 0)
            {
                this.battleBagScript.ShowItemDesc(this.datas[index]);
            }
            else
            {
                this.battleBagScript.ShowItemDesc(null);
            }
        }
    }
    ...
    void ShowItemDesc(RoleItem roleItem)
    {
