...
public class ActionStrategyLoop : ActionStrategyGeneral
{
    //攻击一般原则，总输出最大
    ...
    {
        if(!new ActionNodeManager(activingRoleGO, allRoleGO, mapGridItems, this)
           ...
           .Execute())
        {
            Debug.Log("执行默认靠近主角策略");
            base.GenerateStrategy(activingRoleGO, allRoleGO, mapGridItems);
        }
    }
